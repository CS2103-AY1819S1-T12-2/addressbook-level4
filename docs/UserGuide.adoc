...
PatientBook is an all-in-one convenience utility for medical professionals in Singapore. It is used to store and manage patient data as well as appointment schedules, and retrieve information about symptoms and drugs.
...
. Copy the downloaded file to the folder you wish to use as the home folder for Patient Book.
...
[start = 5]
. Try typing any command into the command box and press `Enter` to run it:
...
- `add patient` : automatically switches to patient management mode, and creates a prompt for adding a new patient
- `add disease d/Influenza s/Fever` : adds the disease "influenza" to the database, which has the symptom "fever"
- `make appointment at 3pm next week` : schedules a 3 PM appointment for the following week
- `delete appointment scheduled at 3pm next week` : removes appointment scheduled at 3 PM in the following week
- `exit` : exits the app

Refer to Section 3, “Features” for details about each command.
[[Features]]
